...
from algorithm import PPOLagrange
...
def main():
    parser = argparse.ArgumentParser(description="Train a PPOLag agent in Isaac Lab (standalone entry).")
    ...
    parser.add_argument("--use_amp", dest="use_amp", action="store_true", help="Force enable AMP (useful on CUDA).")
    parser.add_argument("--no_amp", dest="use_amp", action="store_false", help="Force disable AMP even on CUDA.")
    ...
    from config.galileo_ppolag_cfg import ALGO_CFG, DEFAULT_TRAIN_CFG
    ...
    algo_class = PPOLagrange

    print(f"[INFO] Initializing {ALGO_NAME}...")

    effective_storage = storage_device or str(device)
    print(f"[INFO] Steps/Env: {steps_per_env}, Batch Size: {batch_size}, Storage: {effective_storage}, AMP: {use_amp}")
