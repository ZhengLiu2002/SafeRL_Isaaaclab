...
try:
    from parkour_isaaclab.envs.mdp.parkours import ParkourEvent  # type: ignore
except Exception:
    class ParkourEvent:  # minimal placeholder to keep imports working
        pass
...
def reward_feet_stumble(
    ...
    # Filter out small forces (air-time or light touch) to avoid division instability or false positives
    contact_mask = torch.norm(net_contact_forces, dim=-1) > 1.0
    
    # XY force > 4 * Z force -> Stumble
    stumble = torch.norm(net_contact_forces[:, :, :2], dim=2) > 4 * torch.abs(net_contact_forces[:, :, 2])
    
    rew = torch.any(stumble & contact_mask, dim=1)
    return rew.float()
